...
Note: to allow access from a page loaded from a file ("null" origin) add "null" to `CORS_ORIGIN_WHITELIST`.  Also, an address with a port spedicified is different than without (eg. `localhost` is not the same as `localhost:8000`).
