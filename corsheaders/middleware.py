...
class CorsMiddleware(MiddlewareMixin):
    ...
    def _https_referer_replace(self, request):
        ...
        if request.is_secure() and origin and 'ORIGINAL_HTTP_REFERER' not in request.META:
            ...
            if not conf.CORS_ORIGIN_ALLOW_ALL and not self.origin_found_in_white_lists(request, origin, origin_parts):
                return
    ...
    def process_response(self, request, response):
        ...
        if (
            ...
            not self.origin_found_in_white_lists(request, origin, origin_parts) and
            not self.origin_found_in_model(origin_parts) and
    ...
    def origin_found_in_white_lists(self, request, origin, origin_parts):
        return (
                (origin_parts.netloc in conf.CORS_ORIGIN_WHITELIST and request.scheme == origin_parts.scheme) or
    ...
    def origin_found_in_model(self, origin_parts):
        if conf.CORS_MODEL is None:
            return False
        model = apps.get_model(*conf.CORS_MODEL.split('.'))
        return model.objects.filter(cors=origin_parts.netloc).exists()
